...
func _ready() -> void:
	...
	_streams = {
		...
		"shout": _make([[392.0, 0.06], [523.0, 0.06], [784.0, 0.22]], 0.3, true),
	}
	# 掛け声の 1 文字ごとの「ピッ」（だんだん高く）
	var pitches := [523.0, 587.0, 659.0, 698.0, 784.0, 880.0]
	for i in pitches.size():
		_streams["blip%d" % i] = _make([[pitches[i], 0.05]], 0.22, false)
